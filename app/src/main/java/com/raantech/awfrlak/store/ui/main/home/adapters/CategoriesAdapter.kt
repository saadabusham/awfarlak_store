...
import com.raantech.awfrlak.store.data.models.home.Category
...
class CategoriesAdapter(
    ...
    override var onSpinnerItemSelectedListener: OnSpinnerItemSelectedListener<Category>? =
        null
    private val compoundPadding: Int = 12
    val spinnerItems: MutableList<Category> = arrayListOf()
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.view.data = spinnerItems[position].title
        if (index == position)
    ...
    override fun notifyItemSelected(index: Int) {
        ...
        this.index = index
        item.title?.let { this.spinnerView.notifyItemSelected(index, it) }
